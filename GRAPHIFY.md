...
- Register the assistant integration after install with `graphify install --platform opencode`.
- The PyPI package is `graphifyy`; the CLI command remains `graphify`.
- On Windows, prefer `uv` or `pipx` because they put `graphify` on `PATH`. If plain `pip` is used and `graphify` is not recognized, add the Python Scripts directory to `PATH` or run `python -m graphify`.
